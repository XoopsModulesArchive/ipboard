...
class ad_stats
{
    ...
    public function show_views()
    {
        ...
        $DB->query(
            "SELECT SUM(t.views) as result_count, t.forum_id, f.name as result_name
        ...
        $SKIN->td_header[] = ['Forum', '40%'];

        $SKIN->td_header[] = ['Result', '50%'];

        $SKIN->td_header[] = ['Views', '10%'];

        //+-------------------------------

        $ADMIN->html .= $SKIN->start_table(
            'Topic Views' . " ({$human_from_date['mday']} {$this->month_names[$human_from_date['mon']]} {$human_from_date['year']} to" . " {$human_to_date['mday']} {$this->month_names[$human_to_date['mon']]} {$human_to_date['year']})"
        );
...
'result_count' => $row['result_count'],
    ...
    public function result_screen($mode = 'reg')
    {
        ...
        $SKIN->td_header[] = ['Date', '20%'];

        $SKIN->td_header[] = ['Result', '70%'];

        $SKIN->td_header[] = ['Count', '10%'];

        //+-------------------------------

        $ADMIN->html .= $SKIN->start_table(
            ucfirst($IN['timescale']) . ' ' . $table . " ({$human_from_date['mday']} {$this->month_names[$human_from_date['mon']]} {$human_from_date['year']} to" . " {$human_to_date['mday']} {$this->month_names[$human_to_date['mon']]} {$human_to_date['year']})"
        );
...
'result_time' => $row['result_time'],
    ...
    public function main_screen($mode = 'reg')
    {
        global $IN, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP;
...
2 => ['act', 'stats'],
        ...
        if ('views' != $mode) {
            $ADMIN->html .= $SKIN->add_td_row(
                [
                    '<b>Time scale</b>',
                    $SKIN->form_dropdown('timescale', [0 => ['daily', 'Daily'], 1 => ['weekly', 'Weekly'], 2 => ['monthly', 'Monthly']]),
                ]
            );
        }
        ...
        $ADMIN->html .= $SKIN->add_td_row(
            [
                '<b>Result Sorting</b>',
                $SKIN->form_dropdown('sortby', [0 => ['asc', 'Ascending - Oldest dates first'], 1 => ['desc', 'Descending - Newest dates first']], 'desc'),
            ]
        );
    ...
    public function make_month()
    {
        $return = [];

        for ($m = 1; $m <= 12; $m++) {
            $return[] = [$m, $this->month_names[$m]];
        }
