...
class ad_mysql
{
    ...
    public function __construct()
    {
        ...
        $this->mysql_version = (int)sprintf('%d%02d%02d', $one, $two, (int)$three);
    ...
    public function do_safe_backup($tbl_name = '')
    {
        ...
        if ('' == $tbl_name) {
            ...
            $create_tbl = (int)$IN['create_tbl'];

            $enable_gzip = (int)$IN['enable_gzip'];
        ...
        if ($do_gzip) {
            ...
            echo $this->str_gzip_header . mb_substr($contents, 0, -4) . $this->gzip_four_chars($crc) . $this->gzip_four_chars($size);
        }
    ...
    public function get_table_sql($tbl, $create_tbl, $skip = 0)
    {
        global $IN, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP;
        ...
        if (1 == $skip) {
            if ($INFO['sql_tbl_prefix'] . 'admin_sessions' == $tbl or $INFO['sql_tbl_prefix'] . 'sessions' == $tbl or $INFO['sql_tbl_prefix'] . 'reg_anti_spam' == $tbl or $INFO['sql_tbl_prefix'] . 'search_results' == $tbl) {
                return $ret;
            }
        }
    ...
    public function sql_strip_ticks($data)
    {
    ...
    public function sql_add_slashes($data)
    {
    ...
    public function show_backup_form()
    {
        global $IN, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP;
...
2 => ['code', 'safebackup'],
    ...
    public function view_sql($sql)
    {
        global $IN, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP;

        $limit = 50;

        $start = '' == (int)$IN['st'] ? 0 : (int)$IN['st'];
...
2 => ['code', 'runsql'],
            ...
            $ADMIN->html .= $SKIN->add_td_row(['<center>' . $SKIN->form_textarea('query', $sql) . '</center>']);
        ...
        if ('' != $DB->error) {
            ...
            $ADMIN->html .= $SKIN->add_td_row([$DB->error]);
        ...
        if (preg_match('/^INSERT|UPDATE|DELETE|ALTER/i', trim($sql))) {
            ...
            $ADMIN->html .= $SKIN->add_td_row(["Query: $sql<br>Executed Successfully"]);

            $ADMIN->html .= $SKIN->end_table();

            $ADMIN->output(); // End output and script
        } elseif (0 === stripos($sql, "SELECT")) {
...
'BASE_URL' => $ADMIN->base_url . '&act=mysql&code=runsql&query=' . urlencode($sql),
        ...
        while (false !== ($r = $DB->fetch_row())) {
            ...
            for ($i = 0; $i < $cnt; $i++) {
                if (1 == $man_query) {
                    ...
                    if (mb_strlen($r[$fields[$i]->name]) > 200) {
                        $r[$fields[$i]->name] = mb_substr($r[$fields[$i]->name], 0, 200) . '...';
                    }
                }

                $rows[] = wordwrap(htmlspecialchars(nl2br($r[$fields[$i]->name]), ENT_QUOTES | ENT_HTML5), 50, '<br>', 1);
            }
    ...
    public function run_tool()
    {
        ...
        if ('DROP' == mb_strtoupper($IN['tool']) || 'CREATE' == mb_strtoupper($IN['tool']) || 'FLUSH' == mb_strtoupper($IN['tool'])) {
            $ADMIN->error("You can't do that, sorry");
        }

        foreach ($tables as $table) {
            $DB->query(mb_strtoupper($IN['tool']) . " TABLE $table");
...
2 => ['code', 'dotool'],
        ...
        if ($this->mysql_version >= 32303) {
            $SKIN->td_header[] = ['Table', '20%'];

            $SKIN->td_header[] = ['Rows', '10%'];

            $SKIN->td_header[] = ['TBL Size', '20%'];

            $SKIN->td_header[] = ['Index Size', '20%'];

            $SKIN->td_header[] = ['Export', '10%'];

            $SKIN->td_header[] = ['<input name="allbox" type="checkbox" value="Check All" onClick="CheckAll();">', '10%'];
            ...
            $SKIN->td_header[] = ['Table', '60%'];

            $SKIN->td_header[] = ['Rows', '30%'];
        ...
        if ($this->mysql_version < 32322) {
            $ADMIN->html .= $SKIN->add_td_basic(
                "<select id='button' name='tool'>
													<option value='optimize'>Optimize Selected Tables</option>
												  </select>
												 <input type='submit' value='Go!' id='button'></form>",
                'center',
                'tdrow2'
            );
        } else {
...
2 => ['code', 'runsql'],
        ...
        $SKIN->td_header[] = ['Table', '30%'];

        $SKIN->td_header[] = ['Rows', '70%'];
    ...
    public function gen_size($val, $li, $sepa)
    {
        $sep = pow(10, $sepa);
    ...
    public function gzip_four_chars($val)
    {
        for ($i = 0; $i < 4; $i++) {
            $return .= chr($val % 256);
