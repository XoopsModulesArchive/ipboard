...
class ad_langs
{
    ...
    public function import()
    {
        ...
        $this->name_translated = str_replace("_", ' ', $this->name_translated);
    ...
    public function set_import()
    {
        ...
        $next_id = ['css' => 0, 'wrap' => 0, 'templates' => 0, 'macro' => 0];
...
'url' => stripslashes($config['url']),
        ...
        if ($FH = fopen($this->work_path . '/' . $new_dir . '/stylesheet.css', 'rb')) {
            ...
            $css_string = $DB->compile_db_update_string(
                [
...
'css_text' => $css,
        ...
        if ($FH = fopen($this->work_path . '/' . $new_dir . '/wrapper.html', 'rb')) {
            ...
            $wrap_string = $DB->compile_db_update_string(
                [
...
'template' => $text,
        ...
        if ($FH = fopen($this->work_path . '/' . $new_dir . '/macro.txt', 'rb')) {
...
'can_remove' => 1,
        ...
        if ($FH = fopen($this->work_path . '/' . $new_dir . '/templates.html', 'rb')) {
        ...
        $skin_string = $DB->compile_db_insert_string(
            [
                'sname' => $new_name,
'sid' => $set['new_id'],
...
'default_set' => 0,
    ...
    public function process_template_group($raw, $setid, $group, $isnew = 0)
    {
        global $IN, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP;
...
'can_remove' => 0,
    ...
    public function check_archive($files)
    {
        if (count($files) > 0) {
    ...
    public function list_current()
    {
        ...
        $SKIN->td_header[] = ['Name', '30%'];

        $SKIN->td_header[] = ['Type', '20%'];

        $SKIN->td_header[] = ['File Name', '30%'];

        $SKIN->td_header[] = ['Import', '10%'];
...
'tmpl' => 'Template set',
    ...
    public function rebuild_phpskin($templates_dir, $skins_dir)
    {
        global $IN, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP;
            ...
            while (false !== ($filename = readdir($handle))) {
                if (('.' != $filename) && ('..' != $filename)) {
                    ...
                    if (preg_match("/\.html$/", $filename)) {
                        ...
                        if ($FHD = fopen($templates_dir . '/' . $filename, 'rb')) {
                        ...
                        if ($fh = fopen($skins_dir . '/' . $name . '.php', 'wb')) {
                            fwrite($fh, $final, mb_strlen($final));
    ...
    public function import_error($error, $next_id)
    {
        global $IN, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP;
