...
class ad_settings
{
    ...
    public function css_search_frames()
    {
    ...
    public function css_preview()
    {
        ...
        foreach ($defs as $bit) {
            ...
            if ('' != $type and '' != $value) {
                ...
                if ('color' == $type or 'background-color' == $type) {
    ...
    public function print_css_searchlinks()
    {
        global $IN, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP, $_GET;
        ...
        while (false !== ($r = $DB->fetch_row())) {
            ...
            $result[$r['group_name']][] = ['suid' => $r['suid'], 'func_name' => $r['func_name']];
        }

        $ADMIN->html .= $SKIN->start_table();

        $ADMIN->html .= $SKIN->add_td_row(['<b>Search Results</b>'], 'title');
        ...
        foreach ($result as $group_name => $sub_array) {
            ...
            $ADMIN->html .= $SKIN->add_td_row(["<b>$group_name</b>"], 'catrow');

            if (is_array($sub_array) and count($sub_array) > 0) {
                foreach ($sub_array as $idx => $data) {
                    $ADMIN->html .= $SKIN->add_td_row(["+ <a href='{$SKIN->base_url}&act=rtempl&code=preview&suid={$data['suid']}&type=text&hl=" . urlencode($name) . "' target='preview'>{$data['func_name']}</a>"]);
                }
            }
        }
    ...
    public function print_searchlinks()
    {
        ...
        foreach ($result as $group_name => $sub_array) {
            ...
            if (is_array($sub_array) and count($sub_array) > 0) {
                foreach ($sub_array as $idx => $data) {
                    $ADMIN->html .= $SKIN->add_td_row(["+ <a href='{$SKIN->base_url}&act=rtempl&code=preview&suid={$data['suid']}&type=text&hl=" . urlencode(stripslashes($_POST['search'])) . "' target='preview'>{$data['func_name']}</a>"]);
                }
            }
        }
...
3 => ['suid', $IN['suid']],
    ...
    public function compare_frames()
    {
        global $IN, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP;
    ...
    public function print_compare_top()
    {
        global $IN, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP;

        $content = 'orig' == $IN['content'] ? 'Original Template' : 'Current Template';
    ...
    public function print_compare_new()
    {
        global $IN, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP;
...
3 => ['suid', $IN['suid']],
    ...
    public function macro_two()
    {
        ...
        if (preg_match("/&lt;\{(\S+)\}&gt;/", $IN['lookup'], $match)) {
            ...
            $macro = $match[1];
        } elseif (preg_match("/{ibf\.(skin|lang|vars|member)\.(\w+)}$/", $IN['lookup'], $match)) {
            $is_valid = 1;
        ...
        $ADMIN->html .= $SKIN->add_td_row(['<b>Entered Macro</b>', $IN['lookup']]);

        $ADMIN->html .= $SKIN->add_td_row(['<b>Macro Type</b>', $result]);

        $ADMIN->html .= $SKIN->add_td_row(['<b>Result</b><br>May be blank if no info', $answer]);
