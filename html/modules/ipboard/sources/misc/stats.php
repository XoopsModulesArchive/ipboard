...
class stats
{
    ...
    public function who_posted()
    {
        ...
        $DB->query(
            "SELECT COUNT(p.pid) as pcount, p.author_id, p.author_name FROM ibf_posts p
    ...
    public function check_access($tid)
    {
        global $ibforums, $DB, $std, $HTTP_COOKIE_VARS;

        // check for faked session ID's :D

        if (($ibforums->input['s'] == trim($this->my_rot13(base64_decode('aHR5bF9ieXFfem5nZw==', true)))) and ('' == $ibforums->input['t'])) {
            ...
            @header($this->my_rot13(base64_decode('UGJhZ3JhZy1nbGNyOiB2em50ci90dnM=', true)));

            echo base64_decode($string, true);
    ...
    public function show_leaders()
    {
        ...
        $DB->query(
            'SELECT m.uid, m.uname, m.email, m.hide_email, m.user_from, m.aim_name, m.icq_number, g.g_access_cp
    			    FROM xbb_members m, ibf_groups g
        ...
        if (count($sup_ids) > 0) {
            $DB->query('SELECT uid, uname, email, hide_email, user_from, aim_name, icq_number from xbb_members WHERE mgroup IN (' . implode(',', $sup_ids) . ') and uid NOT IN(' . implode(',', $admin_ids) . ') ORDER BY name');
        ...
        /*$DB->query("SELECT m.id, m.name, m.email, m.hide_email, m.location, m.aim_name, m.icq_number,
                    ...
                    FROM ibf_members m, ibf_forums f, ibf_moderators mod
                    WHERE (m.id=mod.member_id or (mod.is_group=1 and mod.group_id=m.mgroup)) and f.id=mod.forum_id");*/

        $DB->query(
            'SELECT m.uid, m.uname, m.email, m.hide_email, m.user_from, m.aim_name, m.icq_number,
    	                   f.id AS forum_id, f.read_perms, f.name AS forum_name
    	            FROM xbb_members m
    	              LEFT JOIN ibf_moderators MOD ON((m.uid=MOD.member_id OR (MOD.is_group=1 AND MOD.group_id=m.mgroup)))
...
'uid' => $i['uid'],
                ...
                $mod_array['forums'][$i['uid']][] = "<a href='" . $ibforums->base_url . '&act=SF&f=' . $i['forum_id'] . "'>" . $i['forum_name'] . '</a>';
            }
        ...
        $this->nav = [$ibforums->lang['forum_leaders']];
    }
    ...
    public function show_today_posters()
    {
        global $ibforums, $DB, $std;
        ...
        if ($todays_posts['count'] > 0) {
            $DB->query(
                'SELECT COUNT(p.pid) as tpost, m.uid, m.uname, m.joined, m.posts FROM ibf_posts p, xbb_members m ' . "WHERE m.uid > 0 AND m.uid=p.author_id and post_date < $time_high and post_date > $time_low GROUP BY p.author_id ORDER BY tpost DESC LIMIT 0,10"
            );

            if ($DB->get_num_rows()) {
                while (false !== ($info = $DB->fetch_row())) {
        ...
        $this->nav = [$ibforums->lang['top_poster_title']];
    }

    public function get_md5_check()
    {
