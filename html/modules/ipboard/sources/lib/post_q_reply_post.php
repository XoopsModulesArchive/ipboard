...
class post_functions extends Post
{
    ...
    public function process($class)
    {
        ...
        if ($ibforums->input['QPost']) {
            $this->quoted_post['post'] = $class->parser->convert(
                [
...
'EXTRA' => '(' . $this->quoted_post['AUTHOR']['name'] . ' @ ' . $this->quoted_post['DATE'] . ')',
    ...
    public function add_reply($class)
    {
        ...
        if (mb_strlen($class->forum['last_title']) > 30) {
            $class->forum['last_title'] = mb_substr($class->forum['last_title'], 0, 27) . '...';
        }
...
'start_date' => time(),
    ...
    public function show_form($class)
    {
        ...
        if ($class->obj['preview_post']) {
            $class->output .= $class->html->preview($class->parser->convert(['TEXT' => $class->dump_quote . $this->post['post'], 'CODE' => $class->forum['use_ibc'], 'SMILIES' => $ibforums->input['enableemo'], 'HTML' => $class->forum['use_html']]));
        }
...
3 => ['p', $ibforums->input['p']],
        ...
        $class->output = preg_replace('/<!--QUOTE BOX-->/', (string)$quote_box, $class->output);
