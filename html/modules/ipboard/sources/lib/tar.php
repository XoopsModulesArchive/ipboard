...
class tar
{
    public $tar_header_length = '512';

    public $tar_unpack_header = 'a100filename/a8mode/a8uid/a8gid/a12size/a12mtime/a8chksum/a1typeflag/a100linkname/a6magic/a2version/a32uname/a32gname/a8devmajor/a8devminor/a155/prefix';

    public $tar_pack_header = 'A100 A8 A8 A8 A12 A12 A8 A1 A100 A6 A2 A32 A32 A8 A8 A155';
...
'remove_original_files' => 0,
    ...
    public function new_tar($tarpath, $tarname)
    {
    ...
    public function over_write_existing()
    {
        $this->attributes['over_write_existing'] = 1;
    }

    public function over_write_newer()
    {
        $this->attributes['over_write_newer'] = 1;
    }

    public function remove_tar_file()
    {
        $this->attributes['remove_tar_file'] = 1;
    }

    public function remove_original_files()
    {
    ...
    public function current_dir($dir = '')
    {
    ...
    public function list_files($advanced = '')
    {
...
'mode' => mb_substr(decoct($d['mode']), -4),
    ...
    public function extract_files($to_dir, $files = '')
    {
        $this->error = '';
        ...
        foreach ($in_files as $k => $file) {
            ...
            if (is_array($files)) {
                if (!in_array($file['name'], $files, true)) {
                    continue;
                }
            }
            ...
            if ((0 == $file['typeflag']) or (!$file['typeflag']) or ('' == $file['typeflag'])) {
                if ($FH = fopen($file_name, 'wb')) {
                    fwrite($FH, $file['data'], mb_strlen($file['data']));

                    fclose($FH);
                } else {
                    $this->warnings[] = "Could not write data to $file_name";
                }
            } elseif (5 == $file['typeflag']) {
    ...
    public function add_files($files, $root_path = '')
    {
...
'data' => $data,
    ...
    public function write_tar()
    {
        ...
        fwrite($FH, $tardata, mb_strlen($tardata));
...
'data' => $data,
    ...
    public function internal_build_string($string = '', $times = 0)
    {
        $return = '';
