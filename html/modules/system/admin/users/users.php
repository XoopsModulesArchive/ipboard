...
// $Id: users.php,v 1.10 2003/04/02 22:31:42 mvandam Exp $
//  ------------------------------------------------------------------------ //
...
function displayUsers()
{
    ...
    $nav = new XoopsPageNav($usercount, 500, $userstart, 'userstart', 'fct=users');
    ...
    require XOOPS_ROOT_PATH . '/modules/system/admin/users/userform.php';
...
function modifyUser($user)
{
    ...
    if (is_object($user)) {
        if (!$user->isActive()) {
            xoops_confirm(['fct' => 'users', 'op' => 'reactivate', 'uid' => $user->getVar('uid')], 'admin.php', _AM_NOTACTIVE);
        ...
        $sig_cbox_value = (1 == $user->getVar('attachsig')) ? 1 : 0;
...
function updateUser(
    ...
    $user_viewemail = (isset($user_viewemail) && 1 == $user_viewemail) ? 1 : 0;
...
function synchronize($id, $type)
{
    ...
    switch ($type) {
        case 'user':
            ...
            // ipb count post
            $total_posts = 0;
            $sql = 'SELECT posts AS total FROM ' . $xoopsDB->prefix('users') . " WHERE uid = $id";
